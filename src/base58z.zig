...
pub const Base58zError = error{ CannotConcatStrings, BadChecksum, DecodedTooShort, EncodedIsEmpty, DecodedIsEmpty, InvalidCharacter, CannotAllocateConcatBuffer, CharacterOutOfRange, CarryNotZero, CannotAllocateBuffer, CannotReallocate, BufferTooSmall, ConcatBufferTooSmall };
...
// decode -----------------------------------------------------------------------------------------
pub fn decodeWithBuffer(buffer: []u8, encoded: []const u8) Base58zError![]const u8 {
    if (encoded.len == 0) return Base58zError.EncodedIsEmpty;
    if (buffer.len < encoded.len) return Base58zError.BufferTooSmall;
    std.mem.set(u8, buffer, 0);
    var length: usize = 0;
    for (encoded) |r| {
        if (r >= digits.len) return Base58zError.CharacterOutOfRange;
        var carry: u32 = digits[r];
        if (carry == 255) return Base58zError.InvalidCharacter;
        for (buffer[0..length], 0..) |b, i| {
            carry += @as(u32, b) * 58;
...
pub fn decodeWithAllocator(allocator: Allocator, encoded: []const u8) Base58zError![]const u8 {
    if (encoded.len == 0) return Base58zError.EncodedIsEmpty;
    const buffer = allocator.alloc(u8, getDecodedLengthUpperBound(encoded.len)) catch return Base58zError.CannotAllocateBuffer;
    errdefer allocator.free(buffer);
    const decoded = try decodeWithBuffer(buffer, encoded);
    _ = allocator.realloc(buffer, decoded.len) catch return Base58zError.CannotReallocate;
    return decoded;
}
...
pub fn comptimeDecode(comptime encoded: []const u8) [comptimeGetDecodedLength(encoded)]u8 {
    comptime {
        ...
        const decoded = decodeWithBuffer(&buffer, encoded) catch |err| {
            @compileError("failed to decode base58z string: '" ++ @errorName(err) ++ "'");
        };
        return decoded[0..].*;
    }
}
...
// decode check -----------------------------------------------------------------------------------
pub fn decodeCheckWithBuffer(buffer: []u8, encoded: []const u8) Base58zError![]const u8 {
    if (encoded.len == 0) return Base58zError.EncodedIsEmpty;
    if (buffer.len < encoded.len) return Base58zError.BufferTooSmall;
    const decoded = try decodeWithBuffer(buffer, encoded);
    if (decoded.len < 4) return Base58zError.DecodedTooShort;
    ...
    if (expected != actual) return Base58zError.BadChecksum;
    return decoded[0..check_start];
}

pub fn decodeCheckWithAllocator(allocator: Allocator, encoded: []const u8) Base58zError![]const u8 {
    if (encoded.len == 0) return Base58zError.EncodedIsEmpty;
...
pub fn comptimeGetDecodedLength(comptime encoded: []const u8) usize {
    comptime {
        ...
        for (encoded) |r| {
            var carry: u32 = digits[r];
            if (carry == 255) @compileError("failed to compute base58z string length: invalid character '" ++ [_]u8{r} ++ "'");
            for (buffer[0..length], 0..) |b, i| {
                carry += @as(u32, b) * 58;
                buffer[i] = @truncate(u8, carry);
                carry >>= 8;
...
// encode -----------------------------------------------------------------------------------------
pub fn encodeWithBuffer(buffer: []u8, decoded: []const u8) Base58zError![]const u8 {
    if (decoded.len == 0) return Base58zError.DecodedIsEmpty;
    if (buffer.len < getEncodedLengthUpperBound(decoded.len)) return Base58zError.BufferTooSmall;
    std.mem.set(u8, buffer, 0);
    var length: usize = 0;
    for (decoded) |r| {
        var carry: u32 = r;
...
pub fn encodeWithAllocator(allocator: Allocator, decoded: []const u8) Base58zError![]const u8 {
    if (decoded.len == 0) return Base58zError.DecodedIsEmpty;
    const buffer = allocator.alloc(u8, getEncodedLengthUpperBound(decoded.len)) catch return Base58zError.CannotAllocateBuffer;
    errdefer allocator.free(buffer);
    const encoded = try encodeWithBuffer(buffer, decoded);
    _ = allocator.realloc(buffer, encoded.len) catch return Base58zError.CannotReallocate;
    return encoded;
}
...
pub fn comptimeEncode(comptime decoded: []const u8) [comptimeGetEncodedLength(decoded)]u8 {
    comptime {
        ...
        const encoded = encodeWithBuffer(&buffer, decoded) catch |err| {
            @compileError("failed to base58z encode string: '" ++ @errorName(err) ++ "'");
        };
...
// encode check -----------------------------------------------------------------------------------
pub fn encodeCheckWithBuffers(buffer: []u8, concat_buffer: []u8, decoded: []const u8) Base58zError![]const u8 {
    if (decoded.len == 0) return Base58zError.DecodedIsEmpty;
    if (buffer.len < getEncodedLengthUpperBound(decoded.len + 4)) return Base58zError.BufferTooSmall;
    if (concat_buffer.len < decoded.len + 4) return Base58zError.ConcatBufferTooSmall;
    const double_sha256 = doubleSha256(decoded);
    var fba = std.heap.FixedBufferAllocator.init(concat_buffer);
    const concatenated = std.mem.concat(fba.allocator(), u8, &[_][]const u8{ decoded, &[_]u8{ double_sha256[0], double_sha256[1], double_sha256[2], double_sha256[3] } }) catch return Base58zError.CannotConcatStrings;
    return try encodeWithBuffer(buffer, concatenated);
}

pub fn encodeCheckWithAllocator(allocator: Allocator, decoded: []const u8) Base58zError![]const u8 {
    if (decoded.len == 0) return Base58zError.DecodedIsEmpty;
    const buffer = allocator.alloc(u8, getEncodedLengthUpperBound(decoded.len + 4)) catch return Base58zError.CannotAllocateBuffer;
    errdefer allocator.free(buffer);
    const double_sha256 = doubleSha256(decoded);
    const concatenated = std.mem.concat(allocator, u8, &[_][]const u8{ decoded, &[_]u8{ double_sha256[0], double_sha256[1], double_sha256[2], double_sha256[3] } }) catch return Base58zError.CannotConcatStrings;
...
pub fn comptimeEncodeCheck(comptime decoded: []const u8) []const u8 {
    comptime {
        ...
        return encodeCheckWithBuffers(&buffer, &concat_buffer, decoded) catch |err| {
            @compileError("failed to base58z check encode string: '" ++ @errorName(err) ++ "'");
        };
    }
}
